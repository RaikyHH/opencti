...
| app:liveness_port            | APP__LIVENESS_PORT             | null (disabled)       | Port for the [liveness probe](liveness-probe.md) HTTP server. Starts immediately on process launch, before platform initialization.                                              |
...
| app:allow_unsecure_http_resources                   | APP__ALLOW_UNSECURE_HTTP_RESOURCES                     | true                                                                                                                               | When set to false, prevents the application from loading insecure (HTTP) resources (images, manifests, connections, objects) |
...
| app:encryption_key                                  | APP__ENCRYPTION_KEY                                    | no default value                                                                                                                   | Encryption key used by platform cryptographic usage. Must be securely generated with `openssl rand -base64 32`               |
| app:rate_protection:max_requests                    | APP__RATE_PROTECTION__MAX_REQUESTS                     | 10000                                                                                                                              | Maximum of request allow per IP and per window duration, 0 will deny all                                                     |
| app:rate_protection:time_window                     | APP__RATE_PROTECTION__TIME_WINDOW                      | 1                                                                                                                                  | Time window for rate protection in seconds                                                                                   |
| app:rate_protection:ip_skip_list                    | APP__RATE_PROTECTION__IP_SKIP_LIST                     | []                                                                                                                                 | JSON array of IP strings allowed to bypass the rate protection                                                               |
| app:rate_protection:ip_skip_ranges                  | APP__RATE_PROTECTION__IP_SKIP_RANGES                   | []                                                                                                                                 | JSON array of CIDR ranges (e.g. `["192.168.0.0/16", "10.0.0.0/8"]`) allowed to bypass the rate protection                                        |
| app:rate_protection:user_agent_skip_prefixes        | APP__RATE_PROTECTION__USER_AGENT_SKIP_PREFIXES         | []                                                                                                                                 | JSON array of User-Agent prefixes (e.g. `["Mozilla"]`). Requests whose User-Agent starts with any prefix are not rate limited (case-insensitive) |
